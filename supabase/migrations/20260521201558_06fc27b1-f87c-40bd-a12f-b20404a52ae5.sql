-- Rankings RPC: per-user metrics for goals/rankings tab
...
BEGIN
  ...
  cur_msgs AS (
    SELECT cm.user_id AS uid, COUNT(*) AS n
      FROM chat_messages cm
     WHERE cm.company_id = v_company
       AND cm.direction = 'outbound'
       AND cm.user_id IS NOT NULL
       AND cm.created_at::date BETWEEN _period_start AND _period_end
     GROUP BY cm.user_id
  ),
  prev_msgs AS (
    SELECT cm.user_id AS uid, COUNT(*) AS n
     ...
     WHERE cm.company_id = v_company
       AND cm.direction = 'outbound'
       AND cm.user_id IS NOT NULL
       AND cm.created_at::date BETWEEN v_prev_start AND v_prev_end
     GROUP BY cm.user_id
  ),
  goals AS (
...
GRANT EXECUTE ON FUNCTION public.get_user_rankings(date, date) TO authenticated;