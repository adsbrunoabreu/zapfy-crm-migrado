CREATE OR REPLACE FUNCTION public.list_lead_budgets(_period_start date, _period_end date, _pipeline_id uuid DEFAULT NULL::uuid, _assigned_to uuid DEFAULT NULL::uuid, _search text DEFAULT NULL::text, _order_by text DEFAULT 'created_at'::text, _order_dir text DEFAULT 'desc'::text, _limit integer DEFAULT 50, _offset integer DEFAULT 0, _status text DEFAULT NULL::text)
 RETURNS jsonb
 LANGUAGE plpgsql
...
BEGIN
  ...
  v_order_col := CASE COALESCE(_order_by,'created_at')
    ...
    WHEN 'numeric_id' THEN 'COALESCE(l.tenant_seq, l.numeric_id)'
  ...
  v_sql := format($q$
    SELECT COALESCE(jsonb_agg(to_jsonb(b)), '[]'::jsonb) FROM (
      SELECT l.id, l.numeric_id, l.tenant_seq, l.name, l.status::text AS status,
             ...
             ps.name AS stage_name, ps.color AS stage_color, ps.stage_type::text AS stage_type,
      ...
      WHERE l.company_id = %L
        ...
        AND (%L::text IS NULL OR %L = '' OR l.name ILIKE '%%'||%L||'%%' OR l.numeric_id::text = %L OR l.tenant_seq::text = %L)
