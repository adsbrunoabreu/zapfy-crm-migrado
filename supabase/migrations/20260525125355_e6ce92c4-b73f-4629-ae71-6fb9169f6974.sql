-- 1) Permitir o status 'sent_persist_failed' já usado pela RPC mark_outbound_failed
ALTER TABLE public.outbound_message_queue
  DROP CONSTRAINT IF EXISTS outbound_message_queue_status_check;
ALTER TABLE public.outbound_message_queue
  ADD CONSTRAINT outbound_message_queue_status_check
  CHECK (status = ANY (ARRAY[
    'pending'::text, 'processing'::text, 'sent'::text,
    'failed'::text, 'dead'::text, 'cancelled'::text,
    'sent_persist_failed'::text
  ]));

-- 2) Higieniza itens travados em 'processing' por timeout do inline dispatch
UPDATE public.outbound_message_queue
   SET status = 'sent_persist_failed',
       error = COALESCE(error, 'stuck_in_processing_recovered'),
       processed_at = COALESCE(processed_at, now()),
       updated_at = now()
 WHERE status = 'processing'
   AND picked_at < now() - interval '2 minutes';

-- 3) Blinda o trigger: nunca aborta o INSERT da mensagem por bookkeeping de ticket
