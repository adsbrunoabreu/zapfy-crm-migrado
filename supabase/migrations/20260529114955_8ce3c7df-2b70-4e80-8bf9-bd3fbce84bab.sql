...
DECLARE
  ...
  _lead_exists boolean := false;
BEGIN
  IF NEW.lead_id IS NOT NULL THEN
    SELECT EXISTS(SELECT 1 FROM public.leads WHERE id = NEW.lead_id) INTO _lead_exists;
  END IF;

  IF TG_OP = 'INSERT' AND _lead_exists THEN
  ...
  ELSIF TG_OP = 'UPDATE' AND _lead_exists THEN
