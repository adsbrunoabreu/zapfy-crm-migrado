...
DECLARE
  ...
  v_won numeric := 0; v_lost numeric := 0; v_open numeric := 0;
  v_cnt_total int := 0; v_cnt_won int := 0; v_cnt_lost int := 0; v_cnt_open int := 0;
  v_received numeric := 0; v_to_receive numeric := 0;
  v_paid_out numeric := 0; v_to_pay numeric := 0;
  v_won_prev numeric := 0; v_received_prev numeric := 0; v_paid_out_prev numeric := 0;
  v_cnt_won_prev int := 0; v_cnt_closed_prev int := 0;
...
BEGIN
  ...
  -- FIX: agrupa por contact_id e party_name; deriva name no SELECT externo
  top_customers_raw AS (
    SELECT
      fe.contact_id,
      fe.party_name,
      SUM(fe.paid_amount) AS value,
    ...
    WHERE fe.company_id = _company_id
      AND fe.kind='receivable' AND fe.status='paid'
      AND (fe.paid_at AT TIME ZONE v_tz)::date BETWEEN v_from AND v_to
    GROUP BY fe.contact_id, fe.party_name
  ),
  top_customers AS (
    SELECT
      COALESCE(c.name, tc.party_name, 'Sem nome') AS name,
      SUM(tc.value) AS value,
      SUM(tc.count) AS count
    FROM top_customers_raw tc
    LEFT JOIN public.contacts c ON c.id = tc.contact_id
    GROUP BY COALESCE(c.name, tc.party_name, 'Sem nome')
    ORDER BY value DESC
    LIMIT 10
  ),
  upcoming AS (
    SELECT
      fe.id, fe.kind, fe.description, fe.due_date,
  ...
  SELECT jsonb_build_object(
    ...
    'kpis', jsonb_build_object(
      'revenue_won', v_won, 'revenue_won_prev', v_won_prev,
      'received', v_received, 'received_prev', v_received_prev,
      'to_receive', v_to_receive,
      'paid_out', v_paid_out, 'paid_out_prev', v_paid_out_prev,
      'to_pay', v_to_pay,
      ...
      'count_total', v_cnt_total, 'count_won', v_cnt_won, 'count_lost', v_cnt_lost, 'count_open', v_cnt_open,
      'open_value', v_open, 'lost_value', v_lost
    ),
