...
-- 1) Trigger: manter leads.closed_at em sincronia com leads.status
...
BEGIN
  IF NEW.status IN ('won','lost') THEN
    ...
    END IF;
  ELSE
    -- reabertura ou status aberto: limpa closed_at
    NEW.closed_at := NULL;
  END IF;
  RETURN NEW;
END;
...
DROP TRIGGER IF EXISTS trg_sync_lead_closed_at_ins ON public.leads;
DROP TRIGGER IF EXISTS trg_sync_lead_closed_at_upd ON public.leads;

CREATE TRIGGER trg_sync_lead_closed_at_ins
  BEFORE INSERT ON public.leads
  FOR EACH ROW EXECUTE FUNCTION public.sync_lead_closed_at();

CREATE TRIGGER trg_sync_lead_closed_at_upd
  BEFORE UPDATE OF status ON public.leads
  FOR EACH ROW
  WHEN (OLD.status IS DISTINCT FROM NEW.status)
  EXECUTE FUNCTION public.sync_lead_closed_at();

-- 2) Backfill closed_at para leads won/lost legados (usa último status_changed do histórico, fallback updated_at)
...
SET closed_at = COALESCE(
  (SELECT max(lh.created_at)
    ...
    WHERE lh.lead_id = l.id
      ...
      AND COALESCE(lh.payload->>'to','') IN ('won','lost')),
  l.updated_at,
  l.created_at
)
WHERE l.status IN ('won','lost') AND l.closed_at IS NULL;

-- 3) Limpar closed_at de leads que NÃO estão won/lost
UPDATE public.leads
SET closed_at = NULL
WHERE status NOT IN ('won','lost') AND closed_at IS NOT NULL;

-- 4) Corrigir KPI total_leads para refletir leads criados no período
...
BEGIN
  ...
  SELECT jsonb_build_object(
    'total_leads', (SELECT count(*) FROM leads_scope WHERE created_at >= _from AND created_at <= _to),
