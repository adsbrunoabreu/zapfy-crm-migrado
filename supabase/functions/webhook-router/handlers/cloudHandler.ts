...
function parseCloudMessage(message: Record<string, unknown>): ParsedMsg {
  const t = String(message.type ?? '')
  const m = (k: string) => message[k] as Record<string, unknown> | undefined
  switch (t) {
    case 'text':
      return { type: 'text', content: (m('text')?.body as string) ?? null, mediaId: null, mediaMime: null, fileName: null }
    case 'image':
      return { type: 'image', content: (m('image')?.caption as string) ?? null, mediaId: (m('image')?.id as string) ?? null, mediaMime: (m('image')?.mime_type as string) ?? null, fileName: null }
    case 'video':
      return { type: 'video', content: (m('video')?.caption as string) ?? null, mediaId: (m('video')?.id as string) ?? null, mediaMime: (m('video')?.mime_type as string) ?? null, fileName: null }
    case 'audio':
      return { type: 'audio', content: null, mediaId: (m('audio')?.id as string) ?? null, mediaMime: (m('audio')?.mime_type as string) ?? null, fileName: null }
    case 'document':
      return { type: 'document', content: (m('document')?.caption as string) ?? null, mediaId: (m('document')?.id as string) ?? null, mediaMime: (m('document')?.mime_type as string) ?? null, fileName: (m('document')?.filename as string) ?? null }
    case 'sticker':
      return { type: 'sticker', content: null, mediaId: (m('sticker')?.id as string) ?? null, mediaMime: (m('sticker')?.mime_type as string) ?? null, fileName: null }
    case 'location':
      return { type: 'location', content: '[location]', mediaId: null, mediaMime: null, fileName: null }
    case 'contacts':
      return { type: 'contact', content: '[contact]', mediaId: null, mediaMime: null, fileName: null }
    case 'reaction':
      return { type: 'reaction', content: (m('reaction')?.emoji as string) ?? null, mediaId: null, mediaMime: null, fileName: null }
    case 'interactive':
      return { type: 'interactive', content: null, mediaId: null, mediaMime: null, fileName: null }
    default:
      return { type: 'unknown', content: null, mediaId: null, mediaMime: null, fileName: null }
  }
}
